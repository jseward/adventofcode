...
def parse_op(s):
    ...
    out_search = re.search("out (\S+)", s)
    if out_search:
        return partial(out, out_search.group(1))
        
    raise ValueError(s)

def is_clock_valid(state):
    next_clock_value = 0
    for c in state['clock']:
        if c != next_clock_value:
            return False
        next_clock_value = 0 if next_clock_value else 1
    return True
...
def run(op_strings, reg_init=None):
    ...
    while state['instruction'] < len(ops):
        ...
        if not is_clock_valid(state):
            break
        if len(state['clock']) > 20:
            break
    return state
...
def run_part_1():
    with open('day25.input', 'rt') as f:
        op_strings = [line.strip() for line in f.readlines()]
    for a in xrange(1000):
        state = run(op_strings, reg_init={'a': a})
        if len(state['clock']) >= 4:
            print "a={} clock={}".format(a, state['clock'])
        if is_clock_valid(state):
            print "VALID! : {}".format(state)

def run_part_2():
    with open('day25.part2.input', 'rt') as f:
        op_strings = [line.strip() for line in f.readlines()]
