...
def cpy(x, y, state, is_tgl):
    ...
    if verbose_file:
        verbose_file.write("{}: cpy {} ({}) -> {}\n".format(state['instruction'], x, value, y))
...
def inc(x, state, is_tgl):
    ...
    state['registers'][x] += 1
    if verbose_file:
        verbose_file.write("{}: inc {} -> ({})\n".format(state['instruction'], x, state['registers'][x]))
    state['instruction'] += 1
...
def jnz(x, y, state, is_tgl):
    ...
    if verbose_file:
        verbose_file.write("{}: jnz {} {}\n".format(state['instruction'], is_not_zero, y))
...
def tgl(x, state, is_tgl):
    ...
    if verbose_file:
        verbose_file.write("{}: tgl {}\n".format(state['instruction'], value))
